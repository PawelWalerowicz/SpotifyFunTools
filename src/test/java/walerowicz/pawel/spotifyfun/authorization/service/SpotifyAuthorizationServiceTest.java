...
class SpotifyAuthorizationServiceTest {
    ...
    @Test
    void constructorShouldThrowAuthorizationExceptionWhenUnsupportedEncodingExceptionsOccurredDuringParamBuilding() {
        try (MockedStatic<URLEncoder> urlEncoderMockedStatic = mockStatic(URLEncoder.class)) {
            ...
            final var anyWebClient = WebClient.builder().build();
            assertThrows(AuthorizationException.class, () -> new SpotifyAuthorizationService(secretLoader,
                            "clientId",
                            "redirectURI",
                            anyWebClient
                    )
